...
public class TcpProxyServer implements Runnable {
    ...
    public TcpProxyServer(int port) throws IOException {
        ...
        Log.i("AsyncTcpServer",String.format("AsyncTcpServer listen on %d success.\n", this.Port& 0xFFFF));
    }
    ...
    void onAccepted(SelectionKey key) {
        ...
        try {
            ...
            if (destAddress != null) {
                ...
                Log.i("socket error",String.format("Error: socket(%s:%d) target host is null.", localChannel.socket().getInetAddress().toString(), localChannel.socket().getPort()));
//                LocalVpnService.Instance.writeLog("Error: socket(%s:%d) target host is null.", localChannel.socket().getInetAddress().toString(), localChannel.socket().getPort());
            ...
            e.printStackTrace();
            Log.i("socket error","Error: remote socket create failed: %s", e);
